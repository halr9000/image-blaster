...
allowed-tools: Read Write Glob Bash(ls *) Bash(node .claude/scripts/project/project-state.mjs *) Bash(node .claude/scripts/project/show-path.mjs *) Bash(node .claude/scripts/world/generate-world.mjs *)
context: fork
...
- If any referenced `.spz`, collider, or panorama URL exists in `world.json`, make sure the matching local file is present before reporting success. Use `--redownload --index N` to fill missing local files from an existing `N-world.json` without starting a new generation.
...
To materialize missing local files from an existing world response, run:

```bash
node .claude/scripts/world/generate-world.mjs --world "$0" --redownload --index <N>
```
